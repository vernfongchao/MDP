...
const AnnouncementForm = ({ edit, setEdit, announcement }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (edit) {
      const editedAnnouncement = await dispatch(
        editAnnouncement({
          id,
      ...
      if (editedAnnouncement.errors) {
        setTitleError(
          editedAnnouncement.errors.title
            ? editedAnnouncement.errors.title[0]
            : ""
        );
        setContentError(
          editedAnnouncement.errors.content
            ? editedAnnouncement.errors.content[0]
            : ""
        );
      } else {
        setEdit(null);
      ...
      const newAnnouncement = await dispatch(
        addAnnouncement({
          staff_id: user.id,
          title,
          content,
      ...
      if (newAnnouncement.errors) {
        setTitleError(
          newAnnouncement.errors.title ? newAnnouncement.errors.title[0] : ""
        );
        setContentError(
          newAnnouncement.errors.content
            ? newAnnouncement.errors.content[0]
            : ""
        );
      } else {
        setEdit(null);
  ...
  return (
    <div className="announcement-form-page-container" onSubmit={handleSubmit}>
      <div className="announcement-form-header-container">
        {edit ? <h1>Edit Announcement</h1> : <h1>Add Announcement</h1>}
      </div>
      <form className="announcement-form-container">
        <div className="announcement-form-title-container">
          <label
            className={
              titleError
                ? "announcement-form-title-label form-error"
                : "announcement-form-title-label"
            }
          >
            Title:
          </label>
          <input
            className={
              titleError
                ? "announcement-input-form form-error-input-border"
        ...
        <div className="announcement-form-content-container">
          <ReactQuill
            className={
              contentError
                ? "announcement-form-content form-error"
        ...
        {content.length ? (
          <div className="annoucement-form-content-tracker-container">
            <span className="annoucement-form-content-tracker-text">
              character length after styling{" "}
              <span style={content.length > 5000 ? { color: "red" } : null}>
                {content.length}
              </span>
              /5000
            </span>
        ...
        {(titleError || contentError) && (
          <div className="announcement-form-errors-container">
            {titleError && (
              <p className="announcement-form-errors">{titleError}</p>
            )}
            {contentError && (
              <p className="announcement-form-errors">{contentError}</p>
            )}
          </div>
        )}
        ...
        {edit ? (
          ...
          <div className="announcement-form-button-container">
            <button className="announcement-buttons announcement-add">
              Add
            </button>
          </div>
        )}
