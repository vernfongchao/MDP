...
const Chat = ({
  ...
  const room = rooms.filter((room) => {
    if (
      (room.staffId1 === user?.id || room.staffId2 === user?.id) &&
  ...
  useEffect(() => {
    ...
    (async () => {
      if (room) {
        ...
        await dispatch(getMessages(room?.id));
  ...
  }, [room, currStaff, dispatch, setIsLoaded]);
  ...
  return (
    <div className="chat-page-container">
      <div>
        <h1 className="chat-header">Message Log</h1>
      </div>
      {isLoaded && room && (
                ...
                {message?.staffId === user?.id ? (
                  <div className="chat-user-edit-content-container">
                          ...
                          <div className="chat-edit-remove-menu">
                            <div className="chat-edit-button-container">
                              <p
                                ...
                                onClick={(e) => handleEdit(e, i, message.id)}
                              >
                                edit
                              </p>
                              <p
                                className="chat-edit-remove-buttons"
                                onClick={(e) => deleteChat(e, message.id)}
                              >
                                remove
                              </p>
                            </div>
                    ...
                    <div className="chat-user-content-edit-container">
                      ...
                      {message.isEdited && (
                        <span className="chat-user-edited-text">edited</span>
                      )}
                    </div>
                  </div>
                ) : (
                  <div className="chat-staff-name-content-container">
                    ...
                    <div>
                      <p className="chat-staff-content">{message.content}</p>
                    </div>
                    {message.isEdited && (
                      <span className="chat-user-edited-text">edited</span>
                    )}
      ...
      {isLoaded && !messages.length && (
        <div className="chat-staff-new-message-container">
          <p>
            ...
            {currStaff?.firstName}. Only the two of you are in this
            conversation.{" "}
          </p>
        </div>
      )}
        ...
        <form className="message-form" onSubmit={sendChat}>
          ...
          {isEdit ? (
            <div className="message-form-submit-button-container">
              <button
                className="message-form-cancel-button"
                type="button"
                onClick={handleCancel}
              >
                Cancel
              </button>
              <button className="message-form-submit-button" type="submit">
                Send
              </button>
            </div>
          ) : (
