...
const Report = ({ index, setIndex, setSearch, report }) => {
  ...
  const handleSubmit = async (e) => {
    if (newReport) {
      ...
      const editReport = await dispatch(
        patchReport({
          id: report?.id,
          title,
          content,
      ...
      <div className="report-detail-container">
        <div className="report-header-container">
          <div className="report-edit-position">
            ...
            {success ? (
              <span className="report-edit-success">{success}</span>
            ) : null}
            {(report || newReport) && (
              <RiIcons.RiSave3Fill
          ...
          <h1 className="report-form-header">Report</h1>
          {report && !newReport && (
            <h3 className="report-form-id">ID: {report?.id}</h3>
          )}
        </div>
        {(report || newReport) && (
          <>
            <div className="report-title-container">
              <label
                className={
                  titleError.length
                    ? "report-title-label report-title-error"
                    : "report-title-label"
                }
              >
                Title:
              ...
              <input
                className={
                  titleError.length
                    ? "report-title-input report-title-input-error"
                    : "report-title-input"
                }
                value={title}
                maxLength="100"
              ...
              {maxTitle && (
                <div className="report-title-max-text-container">
                  <span className="report-title-max-text">{maxTitle}</span>
                </div>
              )}
              {titleError.length
                ? titleError.map((error) => (
                    <span className="report-title-error-text">{error}</span>
                  ))
                : null}
            </div>
            {report && (
